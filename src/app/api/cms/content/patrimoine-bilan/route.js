...
export async function GET() {
  try {
    const query = `
      ...
      FROM cms_content 
      WHERE page_slug = 'patrimoine-bilan'
    `;
    ...
    console.error('Error fetching patrimoine-bilan CMS content:', error);
    return NextResponse.json({
      success: false,
...
export async function POST(request) {
  try {
    ...
    const checkQuery = `
      ...
      WHERE page_slug = 'patrimoine-bilan' AND section_name = ?
    `;
    ...
    if (existing.length > 0) {
      ...
      const insertQuery = `
        ...
        VALUES ('patrimoine-bilan', ?, ?, NOW(), NOW())
      `;
    ...
    console.error('Error saving patrimoine-bilan CMS content:', error);
    return NextResponse.json({
      success: false,
...
export async function DELETE(request) {
  try {
    ...
    console.error('Error deleting patrimoine-bilan CMS content:', error);
    return NextResponse.json({
      success: false,
