...
export async function GET() {
  try {
    const query = `
      ...
      FROM cms_content 
      WHERE page_slug = 'patrimoine-conseils'
    `;
    ...
    console.error('Error fetching patrimoine-conseils CMS content:', error);
    return NextResponse.json({
      success: false,
...
export async function POST(request) {
  try {
    ...
    const checkQuery = `
      ...
      WHERE page_slug = 'patrimoine-conseils' AND section_name = ?
    `;
    ...
    if (existing.length > 0) {
      ...
      const insertQuery = `
        ...
        VALUES ('patrimoine-conseils', ?, ?, NOW(), NOW())
      `;
    ...
    console.error('Error saving patrimoine-conseils CMS content:', error);
    return NextResponse.json({
      success: false,
...
export async function DELETE(request) {
  try {
    ...
    console.error('Error deleting patrimoine-conseils CMS content:', error);
    return NextResponse.json({
      success: false,
