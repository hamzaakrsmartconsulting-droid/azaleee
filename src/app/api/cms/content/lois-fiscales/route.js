    ...
    for (const [sectionName, sectionContent] of Object.entries(content)) {
      await executeQuery(
        `INSERT INTO cms_content (page_name, section_name, content, is_published, created_at, updated_at) 
         ...
         is_published = true,
         updated_at = NOW()`,
        ['lois-fiscales', sectionName, JSON.stringify(sectionContent)]
      );
    }

    return NextResponse.json({ success: true, message: 'Content saved successfully' });
  } catch (error) {
    console.error('Error saving lois-fiscales content:', error);
