...
export async function GET() {
  try {
    const query = `
      ...
      FROM cms_content 
      WHERE page_slug = 'rachat-trimestres'
    `;
    ...
    console.error('Error fetching rachat-trimestres CMS content:', error);
    return NextResponse.json({
      success: false,
...
export async function POST(request) {
  try {
    ...
    const checkQuery = `
      ...
      WHERE page_slug = 'rachat-trimestres' AND section_name = ?
    `;
    ...
    if (existing.length > 0) {
      ...
      const insertQuery = `
        ...
        VALUES ('rachat-trimestres', ?, ?, NOW(), NOW())
      `;
    ...
    console.error('Error saving rachat-trimestres CMS content:', error);
    return NextResponse.json({
      success: false,
...
export async function DELETE(request) {
  try {
    ...
    console.error('Error deleting rachat-trimestres CMS content:', error);
    return NextResponse.json({
      success: false,
