...
export async function GET() {
  try {
    const query = `
      ...
      FROM cms_content 
      WHERE page_slug = 'patrimoine-autre'
    `;
    ...
    console.error('Error fetching patrimoine-autre CMS content:', error);
    return NextResponse.json({
      success: false,
...
export async function POST(request) {
  try {
    ...
    const checkQuery = `
      ...
      WHERE page_slug = 'patrimoine-autre' AND section_name = ?
    `;
    ...
    if (existing.length > 0) {
      ...
      const insertQuery = `
        ...
        VALUES ('patrimoine-autre', ?, ?, NOW(), NOW())
      `;
    ...
    console.error('Error saving patrimoine-autre CMS content:', error);
    return NextResponse.json({
      success: false,
...
export async function DELETE(request) {
  try {
    ...
    console.error('Error deleting patrimoine-autre CMS content:', error);
    return NextResponse.json({
      success: false,
