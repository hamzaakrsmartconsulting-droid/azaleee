...
const defaultContent = {
    ...
    paragraph: "Votre partenaire de confiance en stratégie immobilière depuis plus de 30 ans. Nous vous accompagnons pour valoriser votre patrimoine, sécuriser vos investissements, et bâtir des solutions d'investissement sur mesure, adaptées à vos objectifs.",
...
export default function PlacementsCMSPage() {
  ...
  const [showToast, setShowToast] = useState(false);
    ...
    const loadContentFromDatabase = async () => {
      try {
        const response = await fetch('/api/pages/content?path=/placements&type=cms');
        if (response.ok) {
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[#686868]">Gérez le contenu de la page placements</p>
      </div>
          <div className="flex items-center gap-3">
                  <button
              onClick={handleSave}
