...
export default function CmsLoiPinelPage() {
  ...
  // Loi Pinel sections configuration
  const loiPinelSections = [
    {
      id: 'hero',
      name: 'Section Hero',
    ...
    },
    {
      ...
      description: 'Présentation générale de la loi Pinel',
      fields: [
        { key: 'title', label: 'Titre Principal', type: 'text' },
        { key: 'description', label: 'Description', type: 'textarea' },
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      const response = await fetch('/api/cms/content/loi-pinel', {
        method: 'POST',
        headers: {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-2xl font-bold text-gray-900">CMS - Loi Pinel</h1>
            </div>
            <div className="flex items-center space-x-4">
              <button
                onClick={() => window.open('/fiscalite/loi-pinel', '_blank')}
              ...
              <div className="bg-white rounded-lg shadow">
                <div className="px-6 py-4 border-b border-gray-200">
                  <h2 className="text-lg font-medium text-gray-900">
                    {loiPinelSections.find(s => s.id === editingSection)?.name}
                  </h2>
                </div>
                <div className="p-6">
                  <div className="space-y-6">
                    {loiPinelSections.find(s => s.id === editingSection)?.fields.map((field) => (
                      <div key={field.key}>
