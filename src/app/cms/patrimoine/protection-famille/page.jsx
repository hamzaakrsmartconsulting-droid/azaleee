...
const defaultContent = {
  hero: {
    ...
    primaryButton: "Protéger ma famille",
  ...
  chart: {
    ...
    data: [
      { label: "Couverture moyenne", value: "€200,000" },
      { label: "Taux de protection", value: "95%" },
      { label: "Coût moyen", value: "€2,500/an" },
      { label: "Délai de versement", value: "48h" },
      { label: "Bénéficiaires", value: "Illimités" }
    ],
...
export default function ProtectionFamilleCMSPage() {
  ...
  const [showToast, setShowToast] = useState(false);
    ...
    const loadContentFromDatabase = async () => {
      try {
        const response = await fetch('/api/pages/content?path=/patrimoine/protection-famille&type=cms');
        if (response.ok) {
  ...
  const handleArrayChange = (section, arrayField, idx, field, value) => {
    ...
    try {
      const response = await fetch('/api/pages/content', {
        ...
        body: JSON.stringify({
          pagePath: '/patrimoine/protection-famille',
          pageType: 'cms',
          content: content,
          metadata: {
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-[#112033]">CMS Protection de la Famille</h1>
            <p className="text-[#686868]">Gérez le contenu de la page protection de la famille</p>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={handleSave}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 className="text-lg font-semibold text-[#112033] mb-4">Section Types de Protection</h2>
        <div className="space-y-4">
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-[#686868] mb-2">Types de protection</label>
              ...
              <div key={index} className="border border-gray-200 rounded-lg p-4 mb-4">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      value={type.title} 
                      onChange={(e) => handleArrayChange('protectionTypes', 'types', index, 'title', e.target.value)} 
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      value={type.icon} 
                      onChange={(e) => handleArrayChange('protectionTypes', 'types', index, 'icon', e.target.value)} 
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => handleArrayChange('protectionTypes', 'types', index, 'description', e.target.value)} 
                      rows={3}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                    />
                  </div>
                      ...
                      <div key={featureIndex} className="mb-2">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            handleArrayChange('protectionTypes', 'types', index, 'features', newFeatures);
                          }}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                        />
                      </div>
