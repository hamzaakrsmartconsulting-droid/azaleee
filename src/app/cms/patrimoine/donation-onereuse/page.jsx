...
const defaultContent = {
    hero: {
    title: "Donation onéreuse",
    paragraph: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
    primaryButton: "Évaluer ma donation",
...
export default function DonationOnereuseCMSPage() {
  ...
  const [showToast, setShowToast] = useState(false);
    ...
    const loadContentFromDatabase = async () => {
      try {
        const response = await fetch('/api/pages/content?path=/patrimoine/donation-onereuse&type=cms');
        if (response.ok) {
  ...
  const handleArrayChange = (section, arrayField, idx, field, value) => {
    ...
    try {
      const response = await fetch('/api/pages/content', {
        ...
        body: JSON.stringify({
          pagePath: '/patrimoine/donation-onereuse',
          pageType: 'cms',
          content: content,
          metadata: {
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-[#112033]">CMS Donation Onéreuse</h1>
            <p className="text-[#686868]">Gérez le contenu de la page donation onéreuse</p>
          </div>
          <div className="flex items-center gap-3">
                <button
              onClick={handleSave}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 className="text-lg font-semibold text-[#112033] mb-4">Section Hero</h2>
          ...
          <div className="border-t pt-4">
            <h3 className="text-md font-medium text-[#112033] mb-3">Cartes de comparaison</h3>
            {content.hero.comparison.map((item, index) => (
            ...
            <div>
                  ...
                  <input
                      value={item.title} 
                      onChange={(e) => handleArrayChange('hero', 'comparison', index, 'title', e.target.value)} 
                ...
                <div>
                  ...
                  <input
                      value={item.icon} 
                      onChange={(e) => handleArrayChange('hero', 'comparison', index, 'icon', e.target.value)} 
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                  />
                </div>
                <div>
                    <label className="block text-xs text-[#686868] mb-1">Valeur</label>
                            <input
                      value={item.value} 
                      onChange={(e) => handleArrayChange('hero', 'comparison', index, 'value', e.target.value)} 
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                            />
                          </div>
                          <div>
                    <label className="block text-xs text-[#686868] mb-1">Description</label>
                      ...
                      onChange={(e) => handleArrayChange('hero', 'comparison', index, 'description', e.target.value)} 
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                            />
                          </div>
                        </div>
      ...
      {/* Onerous Process Section */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 className="text-lg font-semibold text-[#112033] mb-4">Section Processus Onéreux</h2>
              <div className="space-y-4">
                <div>
            <label className="block text-sm font-medium text-[#686868] mb-2">Titre de la section</label>
                  <input
              value={content.onerousProcess.title} 
              onChange={(e) => handleChange('onerousProcess', 'title', e.target.value)} 
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                  />
                </div>
            ...
            {content.onerousProcess.steps.map((step, index) => (
              <div key={index} className="border border-gray-200 rounded-lg p-4 mb-4">
                ...
                <div className="space-y-3">
                    ...
                    <label className="block text-xs text-[#686868] mb-1">Titre</label>
                            <input
                      value={step.title} 
                      onChange={(e) => handleArrayChange('onerousProcess', 'steps', index, 'title', e.target.value)} 
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                            />
                          </div>
                          <div>
                    <label className="block text-xs text-[#686868] mb-1">Icône</label>
                            <input
                              value={step.icon}
                      onChange={(e) => handleArrayChange('onerousProcess', 'steps', index, 'icon', e.target.value)} 
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                            />
                          </div>
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => handleArrayChange('onerousProcess', 'steps', index, 'description', e.target.value)} 
                      rows={3}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                          />
                        </div>
                    ...
                    {step.features.map((feature, featureIndex) => (
                      <div key={featureIndex} className="mb-2">
                          ...
                          onChange={(e) => {
                            ...
                            handleArrayChange('onerousProcess', 'steps', index, 'features', newFeatures);
                          }}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                          />
                        </div>
