...
const defaultContent = {
  ...
  chart: {
    ...
    data: [
      { label: "Bilans réalisés", value: "2,500+" },
      { label: "Satisfaction client", value: "98%" },
      { label: "Économies moyennes", value: "€25,000" },
      { label: "Durée d'analyse", value: "30 minutes" },
      { label: "Expertise", value: "15+ ans" }
    ],
...
export default function BilanCMSPage() {
  ...
  const [showToast, setShowToast] = useState(false);
    ...
    const loadContentFromDatabase = async () => {
      try {
        const response = await fetch('/api/pages/content?path=/patrimoine/bilan&type=cms');
        if (response.ok) {
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-[#112033]">CMS Bilan Patrimonial</h1>
            <p className="text-[#686868]">Gérez le contenu de la page bilan patrimonial</p>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={handleSave}
      ...
      {/* Bilan Steps Section */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 className="text-lg font-semibold text-[#112033] mb-4">Section Étapes du Bilan</h2>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-[#686868] mb-2">Titre de la section</label>
            <input
              value={content.bilanSteps.title} 
              onChange={(e) => handleChange('bilanSteps', 'title', e.target.value)} 
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
            />
          </div>
          
          <div>
            <label className="block text-sm font-medium text-[#686868] mb-2">Étapes du bilan</label>
            {content.bilanSteps.steps.map((step, index) => (
              <div key={index} className="border border-gray-200 rounded-lg p-4 mb-4">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      value={step.number} 
                      onChange={(e) => handleArrayChange('bilanSteps', 'steps', index, 'number', e.target.value)} 
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      value={step.title} 
                      onChange={(e) => handleArrayChange('bilanSteps', 'steps', index, 'title', e.target.value)} 
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => handleArrayChange('bilanSteps', 'steps', index, 'description', e.target.value)} 
                      rows={3}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                    />
                  </div>
