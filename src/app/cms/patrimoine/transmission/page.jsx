...
const defaultContent = {
  hero: {
    ...
    primaryButton: "Planifier ma transmission",
  ...
  chart: {
    ...
    data: [
      { label: "Patrimoine moyen", value: "€500,000" },
      { label: "Économies fiscales", value: "60%" },
      { label: "Coût de transmission", value: "€15,000" },
      { label: "Durée de planification", value: "3-6 mois" },
      { label: "Bénéficiaires", value: "4-6 personnes" }
    ],
  ...
  cta: {
    title: "Lorem ipsum dolor sit amet ?",
    paragraph: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
    button: "Planifier ma transmission"
  }
};
...
export default function TransmissionCMSPage() {
  ...
  const [showToast, setShowToast] = useState(false);
    ...
    const loadContentFromDatabase = async () => {
      try {
        const response = await fetch('/api/pages/content?path=/patrimoine/transmission&type=cms');
        if (response.ok) {
  ...
  const handleArrayChange = (section, arrayField, idx, field, value) => {
    ...
    try {
      const response = await fetch('/api/pages/content', {
        ...
        body: JSON.stringify({
          pagePath: '/patrimoine/transmission',
          pageType: 'cms',
          content: content,
          metadata: {
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-[#112033]">CMS Transmission de Patrimoine</h1>
            <p className="text-[#686868]">Gérez le contenu de la page transmission de patrimoine</p>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={handleSave}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 className="text-lg font-semibold text-[#112033] mb-4">Section Méthodes de Transmission</h2>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-[#686868] mb-2">Titre de la section</label>
            <input
              value={content.transmissionMethods.title} 
              onChange={(e) => handleChange('transmissionMethods', 'title', e.target.value)} 
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
            />
          </div>
          
          <div>
            <label className="block text-sm font-medium text-[#686868] mb-2">Méthodes de transmission</label>
            {content.transmissionMethods.methods.map((method, index) => (
              <div key={index} className="border border-gray-200 rounded-lg p-4 mb-4">
                <h4 className="text-sm font-medium text-[#112033] mb-3">Méthode {index + 1}</h4>
                <div className="space-y-3">
                  <div>
                    <label className="block text-xs text-[#686868] mb-1">Titre</label>
                    <input
                      value={method.title} 
                      onChange={(e) => handleArrayChange('transmissionMethods', 'methods', index, 'title', e.target.value)} 
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      value={method.icon} 
                      onChange={(e) => handleArrayChange('transmissionMethods', 'methods', index, 'icon', e.target.value)} 
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => handleArrayChange('transmissionMethods', 'methods', index, 'description', e.target.value)} 
                      rows={3}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                    />
                  </div>
                      ...
                      <div key={featureIndex} className="mb-2">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            handleArrayChange('transmissionMethods', 'methods', index, 'features', newFeatures);
                          }}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#4EBBBD] focus:border-transparent"
                        />
                      </div>
