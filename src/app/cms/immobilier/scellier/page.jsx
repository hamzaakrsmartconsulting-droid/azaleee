...
const defaultContent = {
  hero: {
    title: "Dispositif Scellier – Lorem Ipsum Dolor Sit Amet",
...
export default function ScellierCMS() {
  ...
  const [showToast, setShowToast] = useState(false);
    ...
    const loadContentFromDatabase = async () => {
      try {
        const response = await fetch('/api/pages/content?path=/immobilier/scellier&type=cms');
        if (response.ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-[#112033]">Page Scellier</h1>
            <p className="text-[#686868]">Modifiez le contenu de la page Scellier</p>
          </div>
          <button 
            onClick={handleSave} 
